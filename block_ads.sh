...
curl -sSfL --retry "$MAX_RETRIES" --retry-all-errors https://big.oisd.nl/domainswild | grep -vE '^\s*(#|$)' > oisd_big_domainswild.txt || silent_error "Failed to download the domains list"

# Check if the file has changed
git diff --exit-code oisd_big_domainswild.txt > /dev/null && silent_error "The domains list has not changed"

# Ensure the file is not empty
[[ -s oisd_big_domainswild.txt ]] || error "The domains list is empty"

# Calculate the number of lines in the file
total_lines=$(wc -l < oisd_big_domainswild.txt)
...
split -l ${MAX_LIST_SIZE} oisd_big_domainswild.txt oisd_big_domainswild.txt. || error "Failed to split the domains list"

# Create array of chunked lists
chunked_lists=()
for file in oisd_big_domainswild.txt.*; do
    chunked_lists+=("${file}")
done
...
git add oisd_big_domainswild.txt || error "Failed to add the domains list to repo"
